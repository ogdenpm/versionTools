...
REM  Script for generation of version info suitable for application use
REM  This is a much simplified version that takes the latest commit for files in a given directory
REM  The version string generated is of the form
REM  [branch-]yyyy.mm.dd.(release | sha1)[+][?]
...
REM     if the output file already exists the existing version in it is used with a ? appended unless it has one already
REM     else the version string is set to xxxx.xx.xx.xx ?
REM
REM  To support different programming languages, an optional version.in file can be supplied with the following contents
REM
REM  [versionfile name]     // [] are required but the whole line can be omitted. default is _version.h
REM  versionfile template
REM
REM  Where template is any text but the string @V@ is replaced with the generated git version string
REM  and @D@ is replaced by today's date in the format YYYY-MM-DD HH:MM:SS
REM
REM  Note, to allow the version string to be read by this script, one line should contain the case insensitive
REM  string GIT_VERSION and the first quoted string on the line should be '@V@' or "@V@"
REM  examples
REM     C    -> #define GIT_VERSION "@V@"
REM     PL/M -> declare version(*) BYTE data('@V@'); /* git_version */
REM 
REM  if version.in is not present the output file is _version.h and the default template is
REM  // Autogenerated version file
REM  #define GIT_VERSION    "@V@"
REM
REM  Unless surpressed by the -q flag, the generated version is shown on the console
REM  -w writes a new output file only if the version has changed or the output file is missing
REM  -W write a new output file
REM
REM  Note when commits are made, without using the mkrelease command then the commited output file
...
REM  It is recommended for sharing the mkrelease command is used. This will create a git commit and a
REM  tag of the form dirname-rnnn. Before committing it will also generate a corresponding version string
REM  with the release component set to nnn.
REM  Note nnn is a decimal number without leading zeros.
REM
...
if exist "%VER_FILE%" (
    for /f "delims=" %%A in ('findstr /R /I "\<GIT_VERSION\>" %VER_FILE%') do set VERLINE=%%A
    set VERLINE=!VERLINE:"='!
    set VERLINE=!VERLINE:*'='!
    set OLD_GIT_VERSION=!VERLINE:'=!
    set VERLINE=
)
