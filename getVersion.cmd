...
REM  #define GIT_VERSION "[branch-]yyyy.mm.dd.(release | gsha1)[+][?]"
...
REM     gsha1       is the start of the git SHA1 preceeded by a g
...
if "%OPT:~0,1%" == "-" (
    ...
    IF "%OPT%" == "-t" if not defined TEMPLATE if exist "%~2" set TEMPLATE=%~2 & SHIFT& SHIFT & goto optloop
    goto USAGE
)
if "%OPT%" neq "" (
   set VER_FILE=%OPT%
...
:USAGE
:: --------------------
ECHO usage: %SCRIPTNAME% -v ^| -h ^| [-q] [-w^|-W] [-t template] [file]
ECHO.
...
ECHO  -w          - write header file if version changed
ECHO  -W          - write header file even if version unchanged
ECHO  -t template - template file to be used to write file
ECHO  file        - header file to write defaults to %DEF_VER_FILE%
ECHO.
ECHO  Example pre-build event:
ECHO  CALL $(SolutionDir)scripts\%SCRIPTNAME%.cmd -W _version.h
ECHO.
ECHO.
GOTO :EOF
